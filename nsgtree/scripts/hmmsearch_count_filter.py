...
def main(args=None):
    ...
    queryfaadir = args[0]
    modelscombined = args[1] # e.g. pathtofiles + "models/GVOGuni9.hmm"
    hmmout = args[2] # e.g. pathtofiles + "Fadolivirus_GVOGuni9.out"
    countout = args[3] # summarized hit counts per model from hmmout
    removedtaxa = args[4] # list of removed taxa with info on why removed these were removed
    minmarker = float(args[5]) # percentage of markers to be present for a genome to be included in downstream analysis$
    maxsdup = int(args[6]) # max copy number of any marker, if exceeded respective genome will be excluded
    maxdupl = float(args[7]) # percentage of markers that can be present in multiple copies
    outitol = args[8]
